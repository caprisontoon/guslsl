...
2. 관리자가 설정한 확률대로 구슬이 만들어집니다. 꽝 63%면 꽝 구슬 63개, 상품 A 10%면
   A 구슬 10개가 필드에 오릅니다.
3. 구슬의 출발 위치는 매 판 무작위로 섞입니다.
4. 1등으로 골인한 구슬의 이름이 그대로 당첨 상품이 됩니다. 꽝이면 기본 혜택 강냉이를
   지급합니다.

## 확률이 정확한 이유

물리 엔진을 건드려 결과를 정하지 않습니다. 대신 **확률을 구슬 개수로 환산**하고,
라벨을 출발 칸에 배정할 때만 무작위를 씁니다.

출발 칸에는 유불리가 있지만(아래쪽 칸이 결승선에 가깝습니다), 라벨을 칸에 배정하는
순열이 균등하고 물리와 독립이므로 **P(상품 당첨) = 해당 상품의 구슬 수 / 전체 구슬 수**
가 정확히 성립합니다. 칸의 유불리는 상쇄됩니다.

실제 물리로 검증할 수 있습니다.

```shell
yarn verify:odds 2000    # 2000판을 돌려 설정 확률과 실측을 비교
```
